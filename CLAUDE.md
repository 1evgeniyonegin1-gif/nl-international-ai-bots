...
### 🆕 Добавлено 24 января (ЗАДЕПЛОЕНО):
- **YandexGPT Pro 32K (лучшая модель):**
  - Переключено с `yandexgpt-lite` на `yandexgpt-32k`
  - Грант Яндекса: 4000 руб (~1 млн токенов)
  - Качество генерации значительно улучшено
- **Post-processing для AI (гарантии):**
  - Принудительные hooks — если AI не начал с hook, он вставляется автоматически
  - Валидация эмодзи — проверяется количество и добавляются если мало
  - Лимиты по персонам: crazy=5, friend=4, rebel=3, expert/philosopher=2, tired=1
  - Файл: `content_manager_bot/ai/content_generator.py` методы `_apply_post_processing()`, `_count_emojis()`
- **Фото продуктов (unified_products/):**
  - Исправлен путь: `product_images/` → `unified_products/`
  - Новый метод `_find_product_photo()` ищет: `{brand}/{product}/photos/*.jpg`
  - 485 MB реальных фото продуктов готовы к использованию
  - Файл: `content_manager_bot/utils/product_reference.py`
- **Диалоговый куратор (без reply-кнопок):**
  - Удалены все reply-кнопки (🍎 Здоровье, 💼 Бизнес и др.)
  - /start теперь начинает естественный диалог
  - conversational_funnel.py активирован для определения этапа воронки
  - Файлы: `curator_bot/handlers/commands.py`, `messages.py`
- **Очистка дублирующего кода:**
  - Удалено: `content_manager_bot/mood/` (дублировало shared/persona/)
  - Удалено: `content_manager_bot/hooks/` (дублировало shared/persona/)
- **Оживление персон (ранее):**
  - 6 индивидуальных SYSTEM_PROMPT для каждой персоны
  - Инструкции по эмоциям с конкретными указаниями
...
| `docs/CHANGELOG_2026_01_24.md` | **Изменения за 24 января** (YandexGPT Pro, post-processing) |
...
| `docs/archive/` | Архивные документы (12 файлов) |
