...
### 🆕 Добавлено 24 января (ГОТОВО К ДЕПЛОЮ):
- **Оживление персон:**
  - 6 индивидуальных SYSTEM_PROMPT для каждой персоны (expert, friend, rebel, philosopher, crazy, tired)
  - Принудительные hooks — AI обязан начинать с них ДОСЛОВНО
  - Инструкции по эмоциям с конкретными указаниями (сколько эмодзи, какой тон)
  - Интеграция персона-промптов в content_generator.py
- **RAG в контент-менеджере:**
  - Добавлен метод `_get_knowledge_context()` — подтягивает примеры из базы знаний
  - Образцы постов лидера из `from_telegram/` используются как few-shot
- **Диалоговый куратор (без кнопок):**
  - Новый файл: `curator_bot/funnels/conversational_funnel.py`
  - Автоматическое определение этапа воронки (greeting → discovery → solution → closing)
  - Выявление болей и возражений из текста
  - Интеллектуальное ведение к продукту/бизнесу
  - Скрипты отработки возражений (цена, доверие, время)
- **MediaManager:**
  - Новый модуль: `shared/media/` для стикеров, гифок, эмодзи
  - Интеграция с unified_products/ (фото продуктов)
  - Готовая структура для добавления реальных file_id
- **YandexGPT Pro:**
  - Обновлён .env: `YANDEX_MODEL=yandexgpt` (вместо lite)
  - Контент-менеджер теперь использует YandexGPT
- **Исправлены ссылки:**
  - `@NL_Curator_Bot` → `@nl_mentor1_bot` везде
  - Канал подтверждён: `@nl_international_partners`
...
- unified_products/: 485 MB изображений (вынести на CDN)
