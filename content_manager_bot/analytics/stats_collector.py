...
class StatsCollector:
    ...
    async def collect_post_stats(self, post: Post) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Получаем количество просмотров (для каналов)
            # Примечание: для групп с Topics views может быть недоступен
            views = getattr(message, 'views', 0) or 0

            # Получаем реакции (если есть)
            reactions_data = {}
            total_reactions = 0

            if hasattr(message, 'reactions') and message.reactions:
                for reaction in message.reactions.reactions:
                    emoji = reaction.emoji if hasattr(reaction, 'emoji') else str(reaction.type)
                    count = reaction.count
                    reactions_data[emoji] = count
                    total_reactions += count

            # Получаем количество пересылок (forwards)
            forwards = getattr(message, 'forwards', 0) or 0

            stats = {
                'views': views,
                'reactions': total_reactions,
                'reactions_breakdown': reactions_data,
                'forwards': forwards
            }

            logger.info(f"Собрана статистика для поста {post.id}: {stats}")
            return stats
